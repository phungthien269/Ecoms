...
const envSchema = z.object({
  ...
  PAYMENT_PROVIDER: z.enum(["mock_gateway", "demo_gateway"]).default("mock_gateway"),
  PAYMENT_WEBHOOK_SECRET: z.string().min(1).default("change_me_payment_webhook"),
  PAYMENT_PROVIDER_DISPLAY_NAME: z.string().default("Mock Gateway"),
  PAYMENT_PROVIDER_BASE_URL: z.string().url().optional(),
  PAYMENT_PROVIDER_MERCHANT_CODE: z.string().optional(),
  PAYMENT_BANK_ACCOUNT_NAME: z.string().default("ECOMS MARKETPLACE"),
  PAYMENT_BANK_ACCOUNT_NUMBER: z.string().default("000111222333"),
  PAYMENT_BANK_NAME: z.string().default("Mock Commerce Bank"),
